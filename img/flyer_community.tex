...
\usepackage{fontspec}
\newfontface\genodefont{cyrvetic.ttf}
...
% A4: 21.0 × 29.7 + bleed width
\usepackage[paperwidth=218.5mm, paperheight=306.5mm]{geometry}
...
\begin{tikzpicture}[remember picture,overlay]
	...
	\tikzstyle{section} = [minimum width=218.5mm, outer sep=0]

	\definecolor{techcolor}      {rgb}{0.641,0.726,0.855}
	\definecolor{sculptcolor}    {rgb}{0.624,0.578,0.624}
	\definecolor{ecosystemcolor} {rgb}{0.642,0.825,0.536}
	...
	\path (current page.north)
		node[below=0cm, anchor=north, section, minimum height=7.75cm] (techoutline) {};
	...
	\path (techoutline.north west)
		node[inner sep=2ex, inner xsep=5.5mm, yshift=-5mm, anchor=north west, font=\normalsize \sffamily, color=white,
			 opacity=1, scale=2] (genodetitle) {GENODE Operating System Framework};

	\path (techoutline.north east)
		node[inner sep=2ex, inner xsep=5.5mm, yshift=-5mm, anchor=north east, font=\normalsize \sffamily, color=black,
		     opacity=0.65, scale=2] {Technology};

	\path (sculptoutline.north west)
		node[inner sep=2ex, inner xsep=5.5mm, anchor=north west, font=\normalsize \sffamily, color=white,
			 opacity=1, scale=2] (sculpttitle) {Sculpt OS};

	\path (sculptoutline.north east)
		node[inner sep=2ex, inner xsep=5.5mm, anchor=north east, font=\normalsize \sffamily, color=black,
			 opacity=0.65, scale=2] (showcasetitle) {Showcase};
	...
	% App-specific TCB
	%
	\path (techoutline.east) node[anchor=east, scale=0.75, outer sep=1cm, outer xsep=14.7mm, yshift=-4ex-5mm] (tcb)
	...
	\path (sculpttitle.south west |- sculptpc.south)+(11mm,-9ex) coordinate (banchor);
	...
	\tikzstyle{wwwlink} = [inner ysep=0.7ex, anchor=east, inner xsep=9.2mm, font=\normalsize \sffamily,
	                      color=linkcolor, opacity=1, scale=1.2]

	\path (ecooutline.north west)+(11mm,-8ex) coordinate (banchor);
	...
	\path (current page.south) + (0,1.1cm) node [anchor=south, opacity=0.5]
		{\includegraphics[width=2.5cm]{genode_logo.pdf}};
...
\begin{tikzpicture}[remember picture, overlay]
	...
	\node at (current page.center)
		{\includegraphics[width=12cm]{genode_logo_truly2.pdf}};

	\path (current page.south east)+(-1.1cm,1.8cm) node [anchor=south east] (qrcode)
		{\includegraphics[width=3.5cm]{qrcode_mastadon.pdf}};

	\node [anchor=south east] at (qrcode.south west) {\LARGE\genodefont follow us...};
	\node [anchor=north east, outer ysep=1mm] at (qrcode.south east) {\large\genodefont https://floss.social/@genode};

	\node [anchor=south, inner sep=0pt, yshift=-3pt] at (qrcode.north)
		{\includegraphics[width=1cm]{ameise.png}};
\end{tikzpicture}
