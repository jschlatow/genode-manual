...
\begin{tikzpicture}[remember picture,overlay]
	...
	\definecolor{usercolor}      {rgb}{0.744,0.480,0.581}
	...
	\path (current page.north)
		node[below=0cm, anchor=north, section, minimum height=8cm] (techoutline) {};
	...
	\path (techoutline.south)
		node[below=0cm, anchor=north, section, minimum height=15cm] (sculptoutline) {};
	...
	\path (sculptoutline.south)
		node[below=0cm, anchor=north, section, minimum height=6.9cm] (ecooutline) {};
	...
	\path (techoutline.north west)+(0,-1ex)
		node[inner sep=2ex, anchor=north west, font=\normalsize \sffamily \bfseries, color=black,
			 opacity=0.9, scale=2.5] (genodetitle) {GENODE Operating System Framework};

	\path (techoutline.south east)+(0,-1ex)
		node[inner sep=2ex, anchor=south east, font=\normalsize \sffamily, color=white,
		     opacity=1, scale=2] {Technology};

	\path (sculptoutline.north west)+(0,1ex)
		node[inner sep=2ex, anchor=north west, font=\normalsize \sffamily \bfseries, color=black,
			 opacity=0.9, scale=2.5] (sculpttitle) {Sculpt OS};

	\path (sculptoutline.south east)+(0,-1ex)
		node[inner sep=2ex, anchor=south east, font=\normalsize \sffamily, color=white,
			 opacity=1, scale=2] (showcasetitle) {Showcase};

	\path (sculptoutline.north)+(0,-11ex) node[anchor=north, font=\normalsize \sffamily, color=black, opacity=0.9, scale=1.5]
		{ Sculpting \textbf{your} custom-tailored OS from first principles };

	\path (ecooutline.south east)
		node[inner sep=2ex, anchor=south east, font=\normalsize \sffamily, color=white,
		     opacity=1, scale=2] {Ecosystem};

	\tikzstyle{bullet} = [inner ysep=0.7ex, anchor=north west, font=\normalsize \sffamily,
	                      color=black, opacity=1, scale=1.2]

	\path (techoutline.north west)+(4ex,-2.8cm) coordinate (banchor);
	...
	% App-specific TCB
	%
	\path (techoutline.east) node[anchor=east, scale=0.75, outer sep=1cm, yshift=-2ex] (tcb)
	...
	\path (current page.center |- sculptoutline.north)+(-4ex,-22ex) node[anchor=north east, screenshot] (sculptpc)
		{\includegraphics[width=8.0cm]{sculpt_2020-09-16.png}};

	\path (current page.center |- sculptoutline.north)+(4ex,-22ex) node[anchor=north west, screenshot] (phonesection)
	...
	\path (sculptoutline.north west |- sculptpc.south)+(4ex,-1.5cm) coordinate (banchor);
	...
	\path (ecooutline.north west)+(4ex,-5ex) coordinate (banchor);
